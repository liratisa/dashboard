async function getTicketSatisfaction() {
    try {
        ...
        if (csatValue) csatValue.innerText = `${data.average.toFixed(1)}/5`;
        if (csatBig) csatBig.innerText = data.average.toFixed(1);
        if (totalCsat) totalCsat.innerText = `CSAT — ${total} avaliações este mês`;
...
async function getAllUsers() {
    try {
        ...
        totalUsers.innerText = `Total de ${total} usuários ativos`;
    } catch (err) {
